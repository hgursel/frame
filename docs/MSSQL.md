...
running in any project. An in-flight request finishes or times out before the pause takes effect. Only one enrichment job runs at a time. Model and plugin settings cannot change during generation. Each object is sent separately in a fresh request with no conversation history. Progress names the current object and its position. Glossary terms are also generated one at a time; subject-area requests contain at most 12 representative objects, not the complete catalog.
